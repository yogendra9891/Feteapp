...
class ChristianconnectModelSubscription extends JModelForm
{
	...
	/*function update order status on order table
	 ...
	 function updateChurchStatus()
	 {
		...
		$subscription_status=(int)$this->getState('subscription_status');	
	 	$db = &JFactory::getDBO();
	 	// Create an object for the record we are going to update.
		$object = new stdClass();
		...
		// Must be a valid primary key value.
		$object->id = $churchid;
		$object->subscription_status=$subscription_status; 
		try {
		    // Update their details in the users table using id as the primary key.
	 ...
	 function updateUserAccess()
	 {
	 	...
	 	$churchid=(int)$this->getState('churchid');
	 	$userid=(int)$this->getState('userid');		
 	...
 	/* Methosd to delete data in user church access table
	  * @param mixed
	  * @return true if successful
	  */
	 ...
	 function deleteUserAccess()
	 {
        ...
        $query = $query->clear();
        $query->delete();
		$query->from('#__user_church_access');
