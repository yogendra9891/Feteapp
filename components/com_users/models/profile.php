...
require_once('components/com_users/tables/feteappusers.php');
...
class UsersModelProfile extends JModelForm
{
	...
	public function save($data)
	{
		...
		if (!$user->save())
		{
			...
			//save the other info of feteappuser in feteappuser table..
			$remove = array_flip(array('id')); 
			$output = array_diff_key($data, $remove);
			$feteappuser = $this->getTable('FeteappUsers','UsersTable');
			$feteappuser->load($output['feteappuserid']); 
			$output['userid'] = $user->id; 
			if (!$feteappuser->save($output)) { 
				$this->setError(JText::sprintf('COM_USER_PROFILE_SAVE_FAILED', $feteappuser->getError()));
				return false;
			} 
		}
	...
	/*
	 * Getting the user other data from feteappuser table..
	 * 
	 */
	public function getFeteappdata(){
		$userId = $this->getState('user.id'); 
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
		$query->select('a.*');
		$query->from('#__feteappusers AS a');
